...
describe("Traits contract tests", async () => {
  ...
  it("Add to blacklist negative (not owner)", async () => {
    await expect(
      blacklist.connect(address1).addToBlacklist(address2.address)
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });

  it("Add to blacklist negative (user already in blacklist)", async () => {
    await blacklist.addToBlacklist(address2.address);
    await expect(blacklist.addToBlacklist(address2.address)).to.be.revertedWith(
  ...
  it("Remove from blacklist negative (not owner)", async () => {
    await blacklist.addToBlacklist(address2.address);
    await expect(
      blacklist.connect(address1).removeFromBlacklist(address2.address)
    ).to.be.revertedWith("Ownable: caller is not the owner");
  });
  ...
  it("Remove from blacklist negative (user already removed from blacklist)", async () => {
    ...
    await expect(
      blacklist.removeFromBlacklist(address2.address)
    ).to.be.revertedWith("User is not in blacklist");
  });
