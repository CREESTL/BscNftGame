...
describe("Mining contract tests", async () => {
  ...
  describe("Process 1 - craft", async () => {
    it("craft without artifacts in recipe", async () => {
      ...
      expect(balanceAfter).to.be.equal(
        balanceBefore.add(ethers.BigNumber.from(1))
      );
      expect(await tree.balanceOf(acc1.address)).to.be.equal(
        INIT_RESOURCES_AMOUNT.sub(ethers.BigNumber.from(10))
      );
      expect(await gold.balanceOf(acc1.address)).to.be.equal(
        INIT_RESOURCES_AMOUNT.sub(ethers.BigNumber.from(2))
      );
    });
    ...
    it("craft with artifact in recipe", async () => {
      ...
      expect(await tree.balanceOf(acc1.address)).to.be.equal(
        INIT_RESOURCES_AMOUNT.sub(ethers.BigNumber.from(25))
      );
      expect(await gold.balanceOf(acc1.address)).to.be.equal(
        INIT_RESOURCES_AMOUNT.sub(ethers.BigNumber.from(5))
      );
    });
  });
  ...
  describe("Process 2 - mining", async () => {
    it("mining", async () => {
      ...
      await mining
        .connect(owner)
        .startMining(1, acc1.address, [300, 0, 0], [0, 0, 0, 1, 0, 0]);
      ...
      expect(await berry.balanceOf(acc1.address)).to.be.equal(
        balanceBefore.add(ethers.BigNumber.from(300))
      );
      expect(await artifacts.balanceOf(acc1.address, 4)).to.be.equal(
        ethers.BigNumber.from(1)
      );
      expect(await tools.balanceOf(acc1.address, 2)).to.be.equal(
        ethers.BigNumber.from(1)
      );
      expect(await tools.connect(acc1).getStrength(1)).to.be.equal(
        ethers.BigNumber.from(380)
      );
    });
    ...
    it("2 minings and rewards claim", async () => {
      ...
      expect(await berry.balanceOf(acc1.address)).to.be.equal(
        balanceBefore.add(ethers.BigNumber.from(600))
      );
      expect(await artifacts.balanceOf(acc1.address, 4)).to.be.equal(
  ...
  describe("Process 3 - repair tool", async () => {
    it("repair tool after mining", async () => {
      ...
      await gold
        .connect(acc1)
        .approve(tools.address, ethers.BigNumber.from("4000000000000000000"));
      ...
      expect(strengthAfter).to.be.equal(
        strengthBefore.add(ethers.BigNumber.from(20))
      );
      expect(balanceAfter).to.be.equal(
        balanceBefore.sub(ethers.BigNumber.from("4"))
      );
    });
  });
});
