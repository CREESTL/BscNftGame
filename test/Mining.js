...
describe("Mining contract", () => {
    ...
    async function deploys() {
        ...
        const artifacts = await upgrades.deployProxy(artifactsFactory.connect(signer), [
            tools.address,
            BASE_URI,
            blacklist.address,
        ]);
    ...
    describe("Main functions", () => {
        describe("Start Mining", () => {
            it("Should start mining", async () => {
                ...
                await tools.connect(signer).addTool(
                    maxStrength,
                    miningDuration,
                    energyCost,
                    strengthCost,
                ...
                await gold.connect(signer).approve(
                    tools.address,
                    await gold.balanceOf(signer.address)
                );
                await tree.connect(signer).approve(
                    tools.address,
                    await tree.balanceOf(signer.address)
                );
                let artifactAmount = 15_000_000; // Random number
                for (let artifactType = 1; artifactType <= 6; artifactType++) {
                    await artifacts.connect(signer).mint(
                        artifactType,
                        signer.address,
                        artifactAmount
                    );
                ...
                let encodedRewards = getRewardsHash(
                    resourceWinAmount, artifactWinAmount 
                );
                ...
                await berry.connect(signer).approve(
                    mining.address,
                    await berry.balanceOf(signer.address)
                );
                await tools.connect(signer).setApprovalForAll(
                    mining.address,
                    true
                );

                // Now start mining
                await expect(mining.connect(signer).startMining(
                    1,
                    signer.address,
                    encodedRewards,
                    signature,
