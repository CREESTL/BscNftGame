...
describe("Tools tests", async () => {
    ...
    describe("Mint functions", async () => {
        ...
        it("add tool (raspberry bush)", async () => {
            await expect(tools.addTool(100, 30, 1, 5, 1, [1, 0, 0, 0, 0, 0], "1.json")).to.emit(tools, "AddTool").withArgs(ethers.BigNumber.from(1));
        });

        it("mint without existed tools", async () => {
            await expect(tools.mint(acc1.address, 1, 10)).to.be.revertedWith("Tools: no tools");
        });
        ...
        it("mint to blacklist", async () => {
            ...
            await expect(tools.mint(acc1.address, 1, 10)).to.be.revertedWith("Tools: user in blacklist");
        });
        ...
        it("mint batch without existed tools", async () => {
            await expect(tools.mintBatch(acc1.address, [1, 2, 3], [1, 1, 1], ethers.utils.randomBytes(10))).to.be.revertedWith("Tools: no tools");
        });
        ...
        it("mintBatch to blacklist", async () => {
            await tools.addTool(100, 30, 1, 5, 1, [1, 0, 0, 0, 0, 0], "1.json");
            await blacklist.addToBlacklist(acc1.address);
            await expect(tools.mintBatch(acc1.address, [1], [1], ethers.utils.randomBytes(10))).to.be.revertedWith("Tools: user in blacklist");
        });
        ...
        it("mintBatch (raspberry bush and strawberry bush)", async () => {
            ...
            await tools.mintBatch(acc1.address, [1, 2], [1, 1], ethers.utils.randomBytes(10));
    ...
    describe("Recipes Functions", async () => {
        it("create recipe", async () => {
            await tools.addTool(100, 30, 1, 5, 1, [1, 0, 0, 0, 0, 0], "1.json");
            await expect(tools.setRecipe(1, 50, [0, 0, 0, 0, 0, 0])).to.emit(tools, "RecipeCreatedOrUpdated").withArgs(ethers.BigNumber.from(1), 50, [0, 0, 0, 0, 0, 0]);
        });

        it("create recipe with invalid tool types amount", async () => {
            await expect(tools.setRecipe(1, 50, [0, 0, 0, 0, 0, 0])).to.be.revertedWith("Tools: invalid toolTypes value");
        });
...
/*         it("create recipe with invalid resource amount", async () => {
        ...
        it("create recipe with invalid artifacts amount", async () => {
            await tools.addTool(100, 30, 1, 5, 1, [1, 0, 0, 0, 0, 0], "1.json");
            await expect(tools.setRecipe(1, 50, [0, 0, 0, 0, 0])).to.be.revertedWith("Tools: invalid array size");
        });

        it("get recipe", async () => {
            await tools.addTool(100, 30, 1, 5, 1, [1, 0, 0, 0, 0, 0], "1.json");
            await tools.setRecipe(1, 50, [0, 0, 0, 0, 0, 0]);
            let {resourcesAmount, artifactsAmount} = await tools.getRecipe(ethers.BigNumber.from(1));
        ...
        it("revert safeBatchTransferFrom with Tools: tokenId is unique", async () => {
            ...
            await expect(tools.safeBatchTransferFrom(acc1.address, acc2.address, [1, 2, 3], [1, 2, 1], ethers.utils.randomBytes(10))).to.be.revertedWith("Tools: tokenId is unique")
        })
        ...
        it("safeBatchTransferFrom", async () => {
            ...
            expect(await tools.balanceOf(acc1.address, 2)).to.be.equal(1)
            expect(await tools.balanceOf(acc2.address, 2)).to.be.equal(0)
            await tools.safeBatchTransferFrom(acc1.address, acc2.address, [1, 2, 3], [1, 1, 1], ethers.utils.randomBytes(10))
        ...
        it("setArtifactsAddress", async () => {
            ...
            await expect(tools.connect(acc1).increaseArtifactAmount()).to.be.revertedWith("Tools: caller is not an Artifacts contract")
        ...
        it("revert corrupt with Tools: msg.sender isn't Mining contract", async () => {
            await expect(tools.corrupt(acc1.address, 1, 10)).to.be.revertedWith("Tools: msg.sender isn't Mining contract")
        });
        ...
        it("safe transfer from revert with Tools: tokenId is unique", async () => {
            await tools.addTool(100, 30, 1, 5, 1, [1, 0, 0, 0, 0, 0], "1.json");
            await tools.mint(acc1.address, 1, 1)
            await expect(tools.safeTransferFrom(acc1.address, acc2.address, 1, 2, ethers.utils.randomBytes(10))).to.be.revertedWith("Tools: tokenId is unique")
        });

        it("safe transfer from revert with Tools: tool doesn't exist", async () => {
            await tools.addTool(100, 30, 1, 5, 1, [1, 0, 0, 0, 0, 0], "1.json");
            await tools.mint(acc1.address, 1, 1)
            await expect(tools.safeTransferFrom(acc1.address, acc2.address, 2, 1, ethers.utils.randomBytes(10))).to.be.revertedWith("Tools: tool doesn't exist")
        });
        ...
        it("revert set tool properties with Tools: invalid strength value", async () => {
            await tools.addTool(100, 30, 1, 5, 1, [1, 0, 0, 0, 0, 0], "1.json");
            await expect(tools.setToolProperties(1, 151, 15, 10, 2)).to.be.revertedWith("Tools: invalid strength value");
        });

        it("revert set tool properties with Tools: mining duration must be greather than zero", async () => {
            await tools.addTool(100, 30, 1, 5, 1, [1, 0, 0, 0, 0, 0], "1.json");
            await expect(tools.setToolProperties(1, 150, 0, 10, 2)).to.be.revertedWith("Tools: mining duration must be greather than zero");
        });
    });
    ...
    describe("Repair functions", async () => {
        it("repair tool with 100% strength", async () => {
            ...
            await expect(tools.connect(acc1).repairTool(1)).to.be.revertedWith("Tools: the tool is already strong enough");
        });
